...
function Squircle(t,n){
   let x_t = Math.abs(Math.cos(t))^(2/n) * Math.sign(Math.cos(t));
   let y_t = Math.abs(Math.sin(t))^(2/n) * Math.sign(Math.sin(t));
...
window.onload = async function()
{
   ...
   colours = [];
    ...
    for(let k = 0; k<num_vertices; k++){
       ...
       let scale = (Math.PI/360)*radius;
       radius += 0.05;
       const v = Squircle(t,0.75)
       vertices.push(scale*v[0], scale*v[1]);
